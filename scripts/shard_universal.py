...
class UniversalShardConfig:
    ...
    # New nested partition options
    nested_partitions: bool = False  # Enable nested partition processing
    partition_dir_pattern: str = "part-*"  # Pattern for partition directories
    partition_file_pattern: str = "part_*.json"  # Pattern for partition files within directories
    exclude_patterns: List[str] = field(default_factory=lambda: ["batch_*.json", "*_counts.json", "*.log", "*.txt"])
...
def _process_file_worker(file_info: Tuple[Path, str, Optional[int], Optional[int]], config: UniversalShardConfig) -> Dict[str, int]:
    """Worker function for multiprocessing - needs to be at module level for pickling"""
    file_path, category_name, file_num, partition_num = file_info
    processor = registry.get_processor(config.mode, config)
    return processor.process_file(file_path, category_name, file_num, partition_num)
...
class BaseProcessor:
    ...
    def extract_file_number(self, dir_name: str) -> int:
        """Extract file number from directory name like 'part-00001'"""
    ...
    def process_file(self, file_path: Path, category_name: str = None, file_num: int = None, partition_num: int = None) -> Dict[str, int]:
...
class TextProcessor(BaseProcessor):
    ...
    def create_shards(self, lines: List[str], split_name: str, part_number: int, file_num: int = None, partition_num: int = None) -> List[Tuple[str, List[str]]]:
        ...
        for shard_idx, start_idx in enumerate(range(0, len(lines), self.config.lines_per_shard)):
            ...
            if self.config.nested_partitions and file_num is not None and partition_num is not None:
                # Use embedded naming scheme: part_{filenum}0{partition}_shard_{shard:06d}.txt
                embedded_part_num = f"{file_num}0{partition_num}"
                shard_filename = f"part_{embedded_part_num}_shard_{shard_idx:06d}.txt"
            else:
    ...
    def process_file(self, file_path: Path, category_name: str = None, file_num: int = None, partition_num: int = None) -> Dict[str, int]:
        ...
        try:
            ...
            if file_num is not None and partition_num is not None:
                part_number = f"{file_num}0{partition_num}"
            else:
            ...
            for split_name, split_lines_data in split_lines.items():
                ...
                shards = self.create_shards(split_lines_data, split_name, part_number, file_num, partition_num)
...
class JsonProcessor(BaseProcessor):
    ...
    def create_shards(self, groups: List[dict], metadata: dict, split_name: str, 
                     file_stem: str, source_name: str, file_num: int = None, partition_num: int = None) -> List[Tuple[str, dict, dict]]:
        ...
        for shard_idx, start_idx in enumerate(range(0, len(groups), self.config.groups_per_shard)):
            ...
            # Create filename with embedded naming if using nested partitions
            if self.config.nested_partitions and file_num is not None and partition_num is not None:
                # Use embedded naming scheme: part_{filenum}0{partition}_shard_{shard:06d}.json
                embedded_part_num = f"{file_num}0{partition_num}"
                shard_filename = f"part_{embedded_part_num}_shard_{shard_idx:06d}.json"
            else:
    ...
    def process_file(self, file_path: Path, category_name: str = None, file_num: int = None, partition_num: int = None) -> Dict[str, int]:
        ...
        try:
            ...
            valid_groups = []
            logger.debug(f"processing groups for {file_path}")
            for group in data.get('groups', []):
                processed = self.process_group(group)
                if processed:
                    valid_groups.append(processed)
            logger.debug(f"done processing groups for {file_path}")
            ...
            # Split groups
            logger.debug(f"splitting data for {file_path}")
            split_groups = self.split_data(valid_groups)
            logger.debug(f"done splitting data for {file_path}")
            ...
            elif self.config.nested_partitions and file_num is not None:
                source_name = f"{self.config.target_dataset_name}_file{file_num:05d}"
            else:
            ...
            for split_name, groups in split_groups.items():
                ...
                # Create and write shards
                logger.debug(f"creating shards for {split_name}")
                shards = self.create_shards(groups, metadata, split_name, file_stem, source_name, file_num, partition_num)
                logger.debug(f"done creating shards for {split_name}")
                total_groups_written = 0
                
                logger.debug(f"writing shard files for {split_name} - {file_path}")
                ...
                logger.debug(f"Done writing shard files for {split_name} - {file_path}")
...
class UniversalFileShardingManager:
    ...
    def find_nested_partition_files(self) -> List[Tuple[Path, str, int, int]]:
        """Find files in nested partition directory structure"""
        all_files = []
        
        # Find partition directories matching the pattern
        partition_dirs = [d for d in self.config.input_path.iterdir() 
                         if d.is_dir() and d.match(self.config.partition_dir_pattern)]
        
        logger.info(f"Found {len(partition_dirs)} partition directories")
        
        for partition_dir in sorted(partition_dirs):
            # Extract file number from directory name
            file_num = self.processor.extract_file_number(partition_dir.name)
            logger.debug(f"Processing directory {partition_dir.name} (file_num={file_num})")
            
            # Find partition files within this directory
            partition_files = [f for f in partition_dir.iterdir() 
                             if f.is_file() and f.match(self.config.partition_file_pattern)
                             and not self.processor.should_exclude_file(f)]
            
            logger.debug(f"Found {len(partition_files)} partition files in {partition_dir.name}")
            
            for file_path in sorted(partition_files):
                # Extract partition number from filename
                match = re.search(r'part_(\d+)\.json', file_path.name)
                partition_num = int(match.group(1)) if match else 0
                
                # Add to files list: (file_path, category_name, file_num, partition_num)
                all_files.append((file_path, None, file_num, partition_num))
        
        return all_files
    
    def find_input_files(self, base_path: Path = None) -> List[Tuple[Path, str, Optional[int], Optional[int]]]:
        """Find all input files matching the pattern"""
        if base_path is None:
            base_path = self.config.input_path
        
        if self.config.nested_partitions:
            return self.find_nested_partition_files()
        ...
        for file_path in input_files:
            if file_path.is_file() and not self.processor.should_exclude_file(file_path):
                if self.config.mode == 'text' and file_path.name.startswith('part-'):
                    try:
                        ...
                        valid_files.append((file_path, None, None, None))
    ...
    def find_nested_files(self) -> List[Tuple[Path, str, Optional[int], Optional[int]]]:
        ...
        for category_dir in self.config.input_path.iterdir():
            if category_dir.is_dir():
                ...
                for file_path, _, file_num, partition_num in category_files:
                    all_files.append((file_path, category_dir.name, file_num, partition_num))
        
        return all_files
    
    def process_files_parallel(self, input_files: List[Tuple[Path, str, Optional[int], Optional[int]]]) -> Dict[str, int]:
        """Process files in parallel using multiprocessing"""
        if self.config.num_workers <= 1:
            # Sequential processing
            results = {'train': 0, 'dev': 0, 'test': 0}
            for file_info in tqdm(input_files, desc="Processing files"):
                file_path, category_name, file_num, partition_num = file_info
                logger.debug(f"processing {file_path}")
                file_results = self.processor.process_file(file_path, category_name, file_num, partition_num)
                logger.debug(f"done processing {file_path}")
    ...
    def run(self):
        ...
        logger.info(f"  Nested partitions: {self.config.nested_partitions}")
        ...
        if not input_files:
            logger.error(f"No valid input files found in {self.config.input_directory}")
            if self.config.nested_partitions:
                logger.error(f"Looking for directories matching '{self.config.partition_dir_pattern}' containing files matching '{self.config.partition_file_pattern}'")
            else:
                logger.error(f"Looking for files matching pattern '{self.config.file_pattern}'")
            return
...
def main():
    ...
    # NEW: Nested partition options
    parser.add_argument("--nested_partitions", action="store_true",
                      help="Enable processing of nested partition directories (e.g., part-00001/part_0.json)")
    parser.add_argument("--partition_dir_pattern", type=str, default="part-*",
                      help="Pattern for partition directories (default: 'part-*')")
    parser.add_argument("--partition_file_pattern", type=str, default="part_*.json",
                      help="Pattern for partition files within directories (default: 'part_*.json')")
    parser.add_argument("--exclude_patterns", type=str, nargs='*', 
                      default=["batch_*.json", "*_counts.json", "*.log", "*.txt"],
                      help="Patterns for files to exclude (default: batch files, logs, etc.)")
    ...
    config = UniversalShardConfig(
        ...
        # New nested partition options
        nested_partitions=args.nested_partitions,
        partition_dir_pattern=args.partition_dir_pattern,
        partition_file_pattern=args.partition_file_pattern,
