...
#     nodes = tree.root.get_subtree_nodes()
#     for node in nodes:
#         assert node.pos_tag != ""  # Should have POS tags from spaCy
#         assert 'morph_features' in node.features  # Should have morph features from spaCy
...
#         assert len(trees) == 3
#         
#         # Check if all trees have proper structure and features
#         for tree in trees:
#             assert tree.root is not None
#             nodes = tree.root.get_subtree_nodes()
#             assert len(nodes) > 0
#             assert all(node.pos_tag != "" for node in nodes)
